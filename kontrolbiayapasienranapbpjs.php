...
<head>
    ...
    <title>Kontrol Biaya Pasien Rawat Inap BPJS - RSUD Pringsewu</title>
    <style>
        * {
...
<body>
    <div class="container">
        <div class="header">
            <h1>💰 Kontrol Biaya Pasien Rawat Inap BPJS</h1>
        </div>
    ...
    ?>
            ...
            <form method="POST" class="filter-form">
                <div class="filter-title">
                    🔍 Filter Kontrol Biaya Pasien Rawat Inap
                </div>
    ...
    if (isset($_POST['filter'])) {
        ...
        // ============================================================
        // QUERY 1: Data utama pasien rawat inap
        // ============================================================
        $query_utama = "SELECT
        ...
        if (!$result_utama) {
            ...
            if ($total_rows > 0) {
                ...
                // ============================================================
                // QUERY 6: Kamar (kamar_inap.ttl_biaya)
                // ============================================================
                $kamar_biaya = [];
                $q_kamar = "SELECT
                    kamar_inap.no_rawat,
                    SUM(kamar_inap.ttl_biaya) AS total_kamar
                FROM kamar_inap
                WHERE kamar_inap.no_rawat IN ($in_clause)
                GROUP BY kamar_inap.no_rawat";
                $res_kamar = mysqli_query($koneksi, $q_kamar);
                if ($res_kamar) {
                    while ($r = mysqli_fetch_assoc($res_kamar)) {
                        $kamar_biaya[$r['no_rawat']] = $r['total_kamar'];
                    }
                }
                ...
                // ============================================================
                // QUERY 10: Perkiraan E-Klaim (Berdasarkan Diagnosa Utama)
                // ============================================================
                $primary_dx = [];
                $q_primary = "SELECT no_rawat, kd_penyakit FROM diagnosa_pasien WHERE no_rawat IN ($in_clause) AND prioritas = 1";
                $res_primary = mysqli_query($koneksi, $q_primary);
                $dx_list = [];
                if ($res_primary) {
                    while ($r = mysqli_fetch_assoc($res_primary)) {
                        $primary_dx[$r['no_rawat']] = $r['kd_penyakit'];
                        $dx_list[] = $r['kd_penyakit'];
                    }
                }

                $eklaim_est = [];
                if (!empty($dx_list)) {
                    $dx_list_unique = array_unique($dx_list);
                    $dx_in_clause = "'" . implode("','", array_map(function($dx) use ($koneksi) {
                        return mysqli_real_escape_string($koneksi, $dx);
                    }, $dx_list_unique)) . "'";

                    $q_est = "SELECT 
                        dp.kd_penyakit,
                        MIN(CAST(ru.disetujui AS DECIMAL(15,2))) AS min_disetujui,
                        MAX(CAST(ru.disetujui AS DECIMAL(15,2))) AS max_disetujui
                    FROM rspsw_umbal ru
                    INNER JOIN diagnosa_pasien dp ON ru.no_rawat = dp.no_rawat AND dp.prioritas = 1
                    INNER JOIN reg_periksa rp ON dp.no_rawat = rp.no_rawat
                    WHERE dp.kd_penyakit IN ($dx_in_clause)
                      AND rp.status_lanjut = 'Ranap'
                      AND ru.disetujui IS NOT NULL AND ru.disetujui != '' AND CAST(ru.disetujui AS DECIMAL(15,2)) > 0
                    GROUP BY dp.kd_penyakit";
                    
                    $res_est = mysqli_query($koneksi, $q_est);
                    if ($res_est) {
                        while ($r = mysqli_fetch_assoc($res_est)) {
                            $eklaim_est[$r['kd_penyakit']] = [
                                'min' => $r['min_disetujui'],
                                'max' => $r['max_disetujui']
                            ];
                        }
                ...
                echo "<div class='table-responsive'><table>
                    <tr>
                        ...
                        <th>Prosedur</th>
                        <th>Total</th>
                        <th>Perkiraan E-Klaim (Min)</th>
                        <th>Perkiraan E-Klaim (Max)</th>
                        <th>Selisih (Min)</th>
                        <th>Selisih (Max)</th>
                    </tr>";
                ...
                foreach ($data_pasien as $row) {
                    ...
                    $val_kamar     = isset($kamar_biaya[$nr]) ? floatval($kamar_biaya[$nr]) : 0;
                    ...
                    $val_eklaim_min = 0;
                    $val_eklaim_max = 0;
                    if (isset($primary_dx[$nr]) && isset($eklaim_est[$primary_dx[$nr]])) {
                        $val_eklaim_min = floatval($eklaim_est[$primary_dx[$nr]]['min']);
                        $val_eklaim_max = floatval($eklaim_est[$primary_dx[$nr]]['max']);
                    }

                    $val_selisih_min = $val_eklaim_min > 0 ? $val_eklaim_min - $val_total : 0;
                    $val_selisih_max = $val_eklaim_max > 0 ? $val_eklaim_max - $val_total : 0;
                    ...
                    $grand_kamar     += $val_kamar;
                    $grand_operasi   += $val_operasi;
                    $grand_total     += $val_total;
                    $grand_eklaim_min += $val_eklaim_min;
                    $grand_eklaim_max += $val_eklaim_max;
                    $grand_selisih_min += $val_selisih_min;
                    $grand_selisih_max += $val_selisih_max;

                    // Format warna selisih
                    $color_selisih_min = $val_selisih_min < 0 ? 'color: #dc3545;' : ($val_selisih_min > 0 ? 'color: #28a745;' : '');
                    $color_selisih_max = $val_selisih_max < 0 ? 'color: #dc3545;' : ($val_selisih_max > 0 ? 'color: #28a745;' : '');
                    ...
                    echo "<tr>
                        ...
                        <td class='angka'>" . number_format($val_kamar, 0, ',', '.') . "</td>
                        ...
                        <td class='angka'>" . ($val_eklaim_min > 0 ? number_format($val_eklaim_min, 0, ',', '.') : '-') . "</td>
                        <td class='angka'>" . ($val_eklaim_max > 0 ? number_format($val_eklaim_max, 0, ',', '.') : '-') . "</td>
                        <td class='angka' style='font-weight:bold; {$color_selisih_min}'>" . ($val_eklaim_min > 0 ? number_format($val_selisih_min, 0, ',', '.') : '-') . "</td>
                        <td class='angka' style='font-weight:bold; {$color_selisih_max}'>" . ($val_eklaim_max > 0 ? number_format($val_selisih_max, 0, ',', '.') : '-') . "</td>
                    </tr>";
                    $no++;
                }
                ...
                echo "<tr class='total-row'>
                    ...
                    <td class='angka'>" . number_format($grand_kamar, 0, ',', '.') . "</td>
                    ...
                    <td class='angka'>" . number_format($grand_eklaim_min, 0, ',', '.') . "</td>
                    <td class='angka'>" . number_format($grand_eklaim_max, 0, ',', '.') . "</td>
                    <td class='angka'>" . number_format($grand_selisih_min, 0, ',', '.') . "</td>
                    <td class='angka'>" . number_format($grand_selisih_max, 0, ',', '.') . "</td>
                </tr>";

                echo "</table></div>";

            } else {
                echo '<div class="no-data">🏥 Tidak ada data pasien rawat inap pada rentang tanggal yang dipilih</div>';
            }
        }
