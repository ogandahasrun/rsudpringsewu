...
if (isset($_POST['action']) && $_POST['action'] === 'panggil_pasien') {
    ...
    if (!isset($_POST['nama_pasien']) || empty($_POST['nama_pasien']) ||
        !isset($_POST['no_resep']) || empty($_POST['no_resep']) ||
    ...
    $kalimat_panggil = 'Pasien atas nama ' . $nama_pasien . ', silakan menuju loket farmasi.';
    $waktu_panggil = date('Y-m-d H:i:s');
    
    // Simpan ke database
    $insert_sql = "INSERT INTO antrian_panggilan_farmasi (nama_pasien, no_resep, no_rawat, kalimat_panggil, waktu_panggil, status_tampil) VALUES (?, ?, ?, ?, ?, 'belum')";
    
    $stmt = $koneksi->prepare($insert_sql);
    if ($stmt) {
        $stmt->bind_param("sssss", $nama_pasien, $no_resep, $no_rawat, $kalimat_panggil, $waktu_panggil);
...
$status_filter = isset($_GET['status']) ? $_GET['status'] : 'ralan';
...
WHERE
    ...
    AND reg_periksa.kd_poli NOT IN ('HDL', 'TLM', 'IGDK')
";
...
$(document).ready(function() {
    ...
    $('.btn-panggil').click(function() {
        ...
        var noResep = button.data('noresep');
