...
if (isset($_GET['action']) && $_GET['action'] === 'get_panggilan') {
    ...
    $sql = "SELECT 
                ...
                waktu_panggil,
                kalimat_panggil
            FROM antrian_panggilan_farmasi 
            WHERE status_tampil = 'belum'
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $update_sql = "UPDATE antrian_panggilan_farmasi 
                       SET status_tampil = 'sudah' 
                       WHERE nama_pasien = ? AND no_resep = ? AND waktu_panggil = ?";
        $stmt = $koneksi->prepare($update_sql);
        if ($stmt) {
            $stmt->bind_param("sss", $row['nama_pasien'], $row['no_resep'], $row['waktu_panggil']);
            $stmt->execute();
            $stmt->close();
        }
        ...
        $debug_sql = "SELECT COUNT(*) as total FROM antrian_panggilan_farmasi";
...
<body>
    ...
    <div class="header">
        <div class="title">DISPLAY ANTRIAN FARMASI</div>
        <div class="subtitle"><?= htmlspecialchars($nama_organisasi) ?></div>
        <!-- Tombol test suara untuk troubleshooting -->
        <button onclick="testSuara()" style="margin-top: 10px; padding: 8px 16px; background: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">
            🔊 Test Suara
        </button>
    </div>
        ...
        <div class="calling-state" id="calling-state">
            ...
            <div class="patient-info">
                ...
                <div class="patient-details">
                    <div><strong>No. Resep :</strong> <span id="patient-resep"></span></div>
                    <div><strong>No. Rawat :</strong> <span id="patient-rawat"></span></div>
                    <div><strong>Waktu Panggil :</strong> <span id="call-time"></span></div>
                </div>
            </div>
            <div class="calling-message">Silakan Menuju Loket Farmasi</div>
        </div>
    </div>
    
    <div class="footer">
        <marquee behavior="scroll" direction="left" scrollamount="5">
            ⚠️ PERHATIAN : Mohon bersabar menunggu - Kami dahulukan pasien DARURAT dan OPERASI 
            🩺 Pasien dengan resep KRONIS dan ITER memerlukan waktu lebih lama untuk verifikasi BPJS 
    ...
    <script>
        ...
        function showCalling(data) {
            ...
            document.getElementById('patient-resep').textContent = data.no_resep;
        ...
        function checkPanggilan() {
            $.ajax({
                url: 'displayantrianfarmasi.php?action=get_panggilan',
                type: 'GET',
                dataType: 'json',
        ...
        function testSuara() {
            ...
            panggilPasien('Test suara berhasil. Sistem panggilan farmasi siap digunakan.');
        }
