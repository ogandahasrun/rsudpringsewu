        ...
        if (!$result) {
            ...
            if ($num_rows > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td class='text-center' data-value='" . $row['jumlah'] . "'>" . number_format($row['jumlah'], 0, ',', '.') . "</td>";
                    echo "<td class='text-center'>" . htmlspecialchars($row['satuan']) . "</td>";
                    echo "<td class='text-right' data-value='" . $row['h_pesan'] . "'>Rp " . number_format($row['h_pesan'], 0, ',', '.') . "</td>";
                    echo "<td class='text-right' data-value='" . $row['subtotal'] . "'>Rp " . number_format($row['subtotal'], 0, ',', '.') . "</td>";
        ...
        function copyTableToClipboard() {
            ...
            rows.forEach(function(row) {
                ...
                cells.forEach(function(cell) {
                    let cellText = cell.hasAttribute('data-value') ? cell.getAttribute('data-value') : cell.textContent.trim();
                    // Clean up the text
                    cellText = String(cellText).replace(/\s+/g, ' ');
                    tsvRow.push(cellText);
                });
                
                if (tsvRow.length > 0) {
                    tsvContent += tsvRow.join('\t') + '\n';
                }
            });
